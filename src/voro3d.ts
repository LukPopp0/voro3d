import createVoro, { type Container, type MainModule as VoroRaw } from "./voro_raw.js";
...
export class Voro3D {
  ...
  get xMin(): number {
    return this.container.xMin;
  }
  set xMin(xMin: number) {
    this.container.xMin = xMin;
  }
  get xMax(): number {
    return this.container.xMax;
  }
  set xMax(xMax: number) {
    this.container.xMax = xMax;
  }
  get yMin(): number {
    return this.container.yMin;
  }
  set yMin(yMin: number) {
    this.container.yMin = yMin;
  }
  get yMax(): number {
    return this.container.yMax;
  }
  set yMax(yMax: number) {
    this.container.yMax = yMax;
  }
  get zMin(): number {
    return this.container.zMin;
  }
  set zMin(zMin: number) {
    this.container.zMin = zMin;
  }
  get zMax(): number {
    return this.container.zMax;
  }
  set zMax(zMax: number) {
    this.container.zMax = zMax;
  }
  get nX(): number {
    return this.container.nX;
  }
  set nX(nX: number) {
    this.container.nX = nX;
  }
  get nY(): number {
    return this.container.nY;
  }
  set nY(nY: number) {
    this.container.nY = nY;
  }
  get nZ(): number {
  ...
  computeCells(points: number[][] | number[], convertToWorld: boolean = false): Cell[] {
    ...
    for (let ci = 0; ci < cellExports.size(); ++ci) {
      ...
      for (let fi = 0; fi < ce.faces.size(); ++fi) {
        const face = ce.faces.get(fi)!;
        const newFace = new Array<number>(face.size());
        for (let ci = 0; ci < face.size(); ++ci) {
          newFace[ci] = face.get(ci)!;
        }
        faces[fi] = newFace;
      }
